...
function populateRoutes(jsonData) {
     ...
     loadButton.addEventListener('click', function () {
          setTimeout(function () {
               ...
               var topRoutesDisplayOptions =
                    document.getElementById('table2Options')
               topRoutesDisplayOptions.disabled = false

               if (checkSelectedValue) {
                    jsonData['Sembawang Polyclinic'].topRoutes.sort(
                         function (a, b) {
                    ...
                    jsonData['Sembawang Polyclinic'].topRoutes.forEach(
                         function (route) {
                              ...
                              var startPoint = route.startPoint.replace(
                                   /_/g,
                                   ' '
                              ...
                              if (isStartPointWithPlus) {
                                   ...
                                   async function convertLatLongToPoiName() {
                                        try {
                                             var poiName =
                                                  await fetchPOINameData(
                                                       'Sembawang Polyclinic',
                                                       latitude,
                                                       longitude
                                                  )
                                             startPointCell.textContent =
                                                  poiName
                                        } catch (error) {
                                             // Handle errors here
                                        }
                    ...
                    var totalSuccessFailedRow =
                         totalSuccessFailedTable.insertRow()
                    ...
                    longestShortestRow.insertCell(0).textContent = (
                         (jsonData['Sembawang Polyclinic']
                              .largestLongestDuration *
                              3600) /
                         60
                    ).toFixed(1)
                    longestShortestRow.insertCell(1).textContent = (
                         (jsonData['Sembawang Polyclinic']
                              .smallestShortestDuration *
                              3600) /
                         60
                    ).toFixed(1)
